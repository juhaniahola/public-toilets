...
router.get('/toilets', async (req, res, _next) => {
  ...
  if(lat && long) {
    await Toilet.find({
      ...
      if(error) { 
        return res.json({ message: 'There was an error', response: null}).status(500)
       }
      ...
      return res.json({ message: `Results within ${kilometers} kilometers`, response: results}).status(200)
    })
  }
})
